...
@section('content')
    ...
    <!-- Title -->
        ...
        <!-- Group -->
        <div class="control-group {!! $errors->has('mnugroup') ? 'has-error' : '' !!} mnugroup">
            <label class="control-label" for="first-name">Group</label>

            <div class="controls">
                {!! Form::select('mnugroup',$menuGroup, $menu->mnugroup, array('class'=>'form-control', 'id' => 'mnugroup', 'value'=>Input::old('mnugroup'))) !!}
                @if ($errors->first('mnugroup'))
                    <span class="help-block">{!! $errors->first('mnugroup') !!}</span>
                @endif
            </div>
        </div>
        <br>
